...
void allocate_memory(list_t *freelist, list_t *alloclist, int pid, int blocksize, int policy)
{
    ...
    node_t *c = freelist->head, *prev = NULL;
    while (c)
    {
        bool fitPolicy = c->blk->end - c->blk->start >= blocksize;

        if (fitPolicy)
        {
            // found a good node
            int pStart = c->blk->start, pEnd = c->blk->start + blocksize - 1;
            int newStart = pEnd + 1, newEnd = c->blk->end;
            // update linked list
            if (prev)
            {
                prev->next = c->next;
            ...
            {
                freelist->head = c->next;
            }

            // free memory
            free(c->blk);
            free(c);
            // add the new node to the allocated list
            block_t *newAllocBlock = malloc(sizeof(block_t)); // create the partition meta data
            newAllocBlock->start = pStart;
            newAllocBlock->end = pEnd;
            newAllocBlock->pid = pid;
            list_add_ascending_by_address(alloclist, newAllocBlock);
            // add the leftover memory back to the free list
            if (newStart <= newEnd)
            {
                block_t *leftOverBlock = malloc(sizeof(block_t)); // create the partition meta data
                leftOverBlock->start = newStart;
                leftOverBlock->end = newEnd;
                leftOverBlock->pid = 0;

                if (policy == 1)
                { // FIRST FIT
                    list_add_to_back(freelist, leftOverBlock);
                }
                else if (policy == 2)
                { // BEST FIT
                    list_add_ascending_by_blocksize(freelist, leftOverBlock);
                }
                else
                { // WORST FIT
                    list_add_descending_by_blocksize(freelist, leftOverBlock);
                }
            }
            return;
        }
...
void deallocate_memory(list_t *alloclist, list_t *freelist, int pid, int policy)
{
    ...
    int indexInAllocList = list_get_index_of_by_Pid(alloclist, pid);

    if (indexInAllocList < 0)
    {
        printf("Error: Can't locate Memory Used by PID: %d\n", pid);
        return;
    }

    block_t *blk = list_remove_at_index(alloclist, indexInAllocList);
    blk->pid = 0;
