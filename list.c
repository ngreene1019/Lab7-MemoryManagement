...
void list_add_ascending_by_address(list_t *l, block_t *newblk)
{
  ...
  // newNode is the new start
  if (!c || newblk->start < c->blk->start)
  {
    newNode->next = l->head;
    l->head = newNode;
  }
  ...
  {
    // insert newNode to the list
    while (c)
    {
      if (c->blk->start < newblk->start && (!c->next || c->next->blk->start > newblk->start))
      {
        node_t *next = c->next;
        c->next = newNode;
        newNode->next = next;
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk)
{
  ...
  // newNode is the new start
  if (!c || compareSize(newblk->end - newblk->start + 1, c->blk))
  {
    newNode->next = l->head;
    l->head = newNode;
  }
  else
  {
    ...
    while (c)
    {
      if (compareSize(c->blk->end - c->blk->start + 1, newblk) && (!c->next || !compareSize(c->next->blk->end - c->blk->start + 1, newblk)))
      {
        node_t *next = c->next;
        c->next = newNode;
        newNode->next = next;
...
void list_add_descending_by_blocksize(list_t *l, block_t *blk)
{
  ...
  else
  {
    ...
    { // only one node in list
      ...
      { // place in front of current node
        newNode->next = l->head;
        l->head = newNode;
      }
      else
      { // place behind current node
        current->next = newNode;
        newNode->next = NULL;
      }
    }
...
void list_coalese_nodes(list_t *l)
{
  ...
  while (c && c->next)
  {
    node_t *tmpNext = c->next;
    if (c->blk->end + 1 == tmpNext->blk->start)
    {
      // merge current & next block
      c->blk->end = tmpNext->blk->end;
      c->next = tmpNext->next;
      // free the next node
      free(tmpNext->blk);
      free(tmpNext);
